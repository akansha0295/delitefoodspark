...
def home(request):
	if 'id' in request.session.keys():
		if request.session['type'] == 'customer':
			foodlist = models.FoodItem.objects.all().order_by('-ordercount')[:5]
			restaurants = models.Restaurant.objects.order_by('name')
			context = {
				'customer':models.Customer.objects.get(email=request.session['id']),
				'restaurants' : restaurants,
				'foodlist' : foodlist,
			}
			return render(request,'foodspark/userhome.html',context)
		elif request.session['type'] == 'restaurant':
			restaurant = models.Restaurant.objects.get(email=request.session['id'])
			query = models.Order.objects.order_by('-pk').all()
			dic = {}
			customer = {}
			for x in query:
...
def signup(request):
	if request.method == 'POST':
		...
		if usertype == 'Customer':
			user = models.Customer(name = name, email = email, phone = phone, address = address)
			user.set_password(user.make_password(password))
			user.save()
			request.session['id'] = email
			request.session['type'] = 'customer'
		elif usertype == 'Restaurant':
			user = models.Restaurant(name= name, email = email, phone = phone, address = address)
			user.set_password(user.make_password(password))
			user.save()
			request.session['id'] = email
			request.session['type'] = 'restaurant'
...
def search(request):
	...
	if searchtype == 'Restaurant':
		restaurants = models.Restaurant.objects.filter(name__contains=searchkey)
	elif searchtype == 'Cuisine':
		foods = models.FoodItem.objects.filter(cuisine__contains=searchkey)
		restaurants = []
		for x in foods:
			if x.resid not in restaurants:
				restaurants.append(x.resid)
	elif searchtype == 'Food':
		foods = models.FoodItem.objects.filter(name__contains=searchkey)
		restaurants = []
		for x in foods:
			if x.resid not in restaurants:
				restaurants.append(x.resid)
		...
		print(searchkey)
		restaurants = models.Restaurant.objects.filter(city__contains=searchkey)
	elif searchtype == 'All':
		restaurants = models.Restaurant.objects.filter(name__contains=searchkey)
		restaurants = list(restaurants)
		foods_cuisine = models.FoodItem.objects.filter(cuisine__contains=searchkey)
		foods = models.FoodItem.objects.filter(name__contains=searchkey)
		for x in foods:
			if x.resid not in restaurants:
				restaurants.append(x.resid)
		for x in foods_cuisine:
			if x.resid not in restaurants:
				restaurants.append(x.resid)
		rescity = models.Restaurant.objects.filter(city__contains=searchkey)
...
def restprofile(request):
	restaurant = models.Restaurant.objects.get(email=request.session['id'])
	fooditems = models.FoodItem.objects.all()
	menu = {}
	for fi in fooditems:
...
def restview(request,restname):
	if 'id' in request.session.keys():
		try:
			customer = models.Customer.objects.get(email=request.session['id'])
			restaurant =models.Restaurant.objects.get(name=restname)
			foodall = models.FoodItem.objects.all()
			fooditems = {}
			for x in foodall:
...
def cart(request):
	if 'id' in request.session.keys():
		if request.method == 'GET':
			...
			orders = {}
			ordersqty = {}
			for q in models.Cart.objects.all():
				if q.customer == models.Customer.objects.get(email=request.session['id']):
					try:
			...
			for x,y in zip(orders,ordersqty):
				o = models.Order(customer=customer,restaurant=x,foodlist=orders[x],foodqty=ordersqty[y],ordertime=datetime.datetime.now(),deliverystatus='p')
				o.calamount()
				o.save()
		...
		for x in foodall:
			if 'food' + str(x.pk) in request.POST.keys():
				if int(request.POST['food' + str(x.pk)]) > 0:
					cartitem = models.Cart(customer = models.Customer.objects.get(email=request.session['id']), fooditem = models.FoodItem.objects.get(pk=x.pk), foodqty= request.POST['food' + str(x.pk)])
					cartitem.save()
		customer = models.Customer.objects.get(email=request.session['id'])
		query = models.Cart.objects.all()
		cart = {}
...
def delivered(request):
	if 'id' in request.session.keys() and request.session['type'] == 'restaurant':
		order = models.Order.objects.get(pk = request.POST['orderid'])
...
def addfooditem(request):
	if 'id' in request.session.keys() and request.session['type'] == 'restaurant':
		...
		food = models.FoodItem(resid=restaurant,name=name,cuisine=cuisine,price=price,course='s',availability_time=datetime.datetime.now())
		food.save()
...
def removefooditem(request):
	if 'id' in request.session.keys() and request.session['type'] == 'restaurant':
		restaurant = models.Restaurant.objects.get(email=request.session['id'])
		food = models.FoodItem.objects.get(pk=request.POST['foodid'])
		food.delete()
		return redirect('/restprofile/')
	else:
		return render(request,"foodspark/login.html")
...
def about(request):
	if 'id' in request.session.keys():
		if request.session['type'] == 'restaurant':
			user = models.Restaurant.objects.get(email=request.session['id'])
		else:
			user = models.Customer.objects.get(email=request.session['id'])
		context = {
			'user': user,
		}
